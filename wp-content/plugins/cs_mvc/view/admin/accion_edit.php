<?php
/**
 * Created by PhpStorm.
 * User: pr0x
 * Date: 10/05/2015
 * Time: 1:40
 */
$model = new AccionViewModel();
$item = $model->getAccion();
?>
<div class="wrap">
    <?php if ($item->id): ?>
        <h2>Editando Acción: <?= __($item->nombre) ?></h2>
    <?php else: ?>
        <h2>Nueva Accion</h2>
    <?php endif ?>
    <form method="post">
        <input type="hidden" name="controller" value="accion.save">
        <input type="hidden" name="id" value="<?= $item->id ?>">
        <table class="form-table">
            <tbody>
            <tr class="wrap">
                <th><label for="user_login">Nombre</label></th>
                <td>
                    <?=HTML::listInputTranslate('nombre',$item->nombre,'input',array(
                        'onblur'=>'setValuesByLang(this)'
                    ))?>
                </td>
            </tr>

            <tr class="wrap">
                <th>
                    <label for="first_name">Descripcion</label>
                    <br>
                </th>
                <td>
                    <?=HTML::listInputTranslate('descripcion',$item->descripcion,'textarea',array(
                        'onblur'=>'setValuesByLang(this)'
                    ))?>
                </td>
            </tr>


            </tbody>
        </table>
        <div class="right">
            <?= HTML::button_primary('Guardar') ?>
            <?= HTML::link_buttonVM('Ver Listado', 'cs_mvc_acciones') ?>
        </div>
    </form>
</div>
<script>
    function setValuesByLang(select_el) {
        var $ = jQuery;
        var $sel = $(select_el);
        var arr = $sel.attr('name').split('_');
        var $el = $('[name='+ arr[0] +']');
        $el.val('');
        $('[name^='+ $el.attr('name') +'_]').each(function (i, item) {
            $item = $(item);
            var arr = $item.attr('name').split('_');
            $el.val($el.val()+'[:'+arr[1]+']'+$item.val());
        })
    }
</script>