...
$item = $model->getTipoExcursion();
?>
<div class="wrap">
    <?php if ($item->id): ?>
        <h2>Editando Tipo de Excursion: <?= __($item->__toString()) ?></h2>
    <?php else: ?>
        <h2>Agregando Tipo de Excursion</h2>
    <?php endif ?>
    <form method="post">
        <input type="hidden" name="controller" value="TipoExcursion.save">
        ...
        <div class="right">
            ...
            <?= HTML::link_buttonVM('Ver Listado', 'cs_mvc_tipo_excursiones') ?>
        </div>
    </form>
</div>
<script>
