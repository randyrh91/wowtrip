...
$item = $model->getTipoDestino();
?>
<div class="wrap">
    <?php if ($item->id): ?>
        <h2>Editando TipoDestino: <?= __($item->__toString()) ?></h2>
    <?php else: ?>
        <h2>Agregando TipoDestino</h2>
    <?php endif ?>
    <form method="post">
        <input type="hidden" name="controller" value="TipoDestino.save">
        ...
        <div class="right">
            ...
            <?= HTML::link_buttonVM('Ver Listado', 'cs_mvc_tipo_destinos') ?>
        </div>
    </form>
</div>
<script>
