 ...
 * User: pr0x
 */
$model = new ServicioViewModel();
?>
<div class="wrap">
<h2>Servicios(<?=count($model->getServicios())?>) <?=HTML::link_buttonVM('+ A&ntilde;adir Nuevo','cs_mvc_servicios','admin/Servicio_edit','Servicio','','add-new-h2')?>
...
<table class="wp-list-table widefat fixed">
    ...
    <?php foreach($model->getServicios() as $item): ?>
        <tr>
			...
			<td><?=HTML::truncate(__( $item->icon ))?></td>
            <td>
                <?=HTML::link_buttonVM('Editar','cs_mvc_servicios','admin/Servicio_edit','Servicio',$item->id)?>
