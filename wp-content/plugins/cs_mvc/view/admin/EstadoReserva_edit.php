...
$item = $model->getEstadoReserva();
?>
<div class="wrap">
    <?php if ($item->id): ?>
        <h2>Editando EstadoReserva: <?= __($item->__toString()) ?></h2>
    <?php else: ?>
        <h2>Agregando EstadoReserva</h2>
    <?php endif ?>
    <form method="post">
        <input type="hidden" name="controller" value="EstadoReserva.save">
        ...
        <div class="right">
            ...
            <?= HTML::link_buttonVM('Ver Listado', 'cs_mvc_estado_reservas') ?>
        </div>
    </form>
</div>
<script>
