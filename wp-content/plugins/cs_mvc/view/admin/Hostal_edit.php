<?php
/**
 * Created by CS-Genarator.
 * User: pr0x
 */
$model = new HostalViewModel();
$item = $model->getHostal();
?>
<div class="wrap">
    <?php if ($item->id): ?>
        <h2>Editando Hostal: <?= __($item->__toString()) ?></h2>
    <?php else: ?>
        <h2>Agregando Hostal</h2>
    <?php endif ?>
    <form method="post">
        <input type="hidden" name="controller" value="Hostal.save">
        <input type="hidden" name="id" value="<?= $item->id ?>">
        <table class="form-table">
            <tbody>
					<tr><td colspan="2">
				<input name="id" type="hidden" value="<?=__($item->id)?>"> 
				</td></tr>
	
					<tr class="wrap">
					<th><label>Nombre</label></th>
					<td>
							<?=HTML::listInputTranslate('nombre',$item->nombre,'input',array(
							'onblur'=>'setValuesByLang(this)'
						))?>
						</td>
				</tr>
	
					<tr class="wrap">
					<th><label>TextoCorto</label></th>
					<td>
							<?=HTML::listInputTranslate('textoCorto',$item->textoCorto,'input',array(
							'onblur'=>'setValuesByLang(this)'
						))?>
						</td>
				</tr>
	
					<tr class="wrap">
					<th><label>Descripcion</label></th>
					<td>
							<?=HTML::listInputTranslate('descripcion',$item->descripcion,'input',array(
							'onblur'=>'setValuesByLang(this)'
						))?>
						</td>
				</tr>
	
					<tr class="wrap">
					<th><label>Direccion</label></th>
					<td>
							<?=HTML::listInputTranslate('direccion',$item->direccion,'input',array(
							'onblur'=>'setValuesByLang(this)'
						))?>
						</td>
				</tr>
	
					<tr class="wrap">
					<th><label>Geopos</label></th>
					<td>
							<?=HTML::listInputTranslate('geopos',$item->geopos,'input',array(
							'onblur'=>'setValuesByLang(this)'
						))?>
						</td>
				</tr>
	
					<tr class="wrap">
					<th><label>CantidadHabitacion</label></th>
					<td>
							<input name="cantidadHabitacion" type="text" value="<?=__($item->cantidadHabitacion)?>"> 
						</td>
				</tr>
	
					<tr class="wrap">
					<th><label>CantidadPersona</label></th>
					<td>
							<input name="cantidadPersona" type="text" value="<?=__($item->cantidadPersona)?>"> 
						</td>
				</tr>
	
					<tr class="wrap">
					<th><label>Precio</label></th>
					<td>
							<input name="precio" type="text" value="<?=__($item->precio)?>"> 
						</td>
				</tr>
	
					<tr class="wrap">
					<th><label>Oferta</label></th>
					<td>
							<input name="oferta" type="text" value="<?=__($item->oferta)?>"> 
						</td>
				</tr>
	
				<tr class="wrap">
					<th><label>DestinoTuristico</label></th>
					<td>
						<?=HTML::dropDownList('Destino_Turistico_id',$model->getDestinoTuristicos(),$item->id)?>
					</td>
				</tr>
		</tbody>
        </table>
        <div class="right">
            <?= HTML::button_primary('Guardar') ?>
            <?= HTML::link_buttonVM('Ver Listado', 'cs_mvc_hostales') ?>
        </div>
    </form>
</div>
<script>
    function setValuesByLang(select_el) {
        var $ = jQuery;
        var $sel = $(select_el);
        var arr = $sel.attr('name').split('_');
        var $el = $('[name='+ arr[0] +']');
        $el.val('');
        $('[name^='+ $el.attr('name') +'_]').each(function (i, item) {
            $item = $(item);
            var arr = $item.attr('name').split('_');
            $el.val($el.val()+'[:'+arr[1]+']'+$item.val());
        })
    }
</script>