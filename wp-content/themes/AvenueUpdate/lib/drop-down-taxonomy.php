...
SCRIPT;
  ...
  echo "<select name=\"{$id}\" id=\"{$id}\" class='form-control input-sm'>";
...
function the_dropdown_taxonomy_no_script($taxonomy) {
    $id = "{$taxonomy}-dropdown-no-script";
    $terms = get_terms($taxonomy, array( 'get' => 'all' ));
    echo "<select name=\"{$id}\" id=\"{$id}\" class='form-control input-sm'>";
    echo '<option value="#"> - Select - </option>';
    foreach($terms as $term) {
